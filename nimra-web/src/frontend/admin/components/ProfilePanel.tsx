      ...
      <div className="profile-panel" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="profile-panel-content">
          ...
          <form className="profile-form" onSubmit={(event) => { event.preventDefault(); void handleProfileSave(); }}>
            ...
            <div className="form-group"><label htmlFor="profile-phone">Phone Number</label><input id="profile-phone" type="tel" value={profileForm.phone} onChange={(event) => { setProfileForm((previous) => ({ ...previous, phone: event.target.value })); if (profileValidationErrors.phone) setProfileValidationErrors((previous) => ({ ...previous, phone: '' })); }} className={`form-input ${profileValidationErrors.phone ? 'form-input-error' : ''}`} placeholder="+91 99999 99999"/>{profileValidationErrors.phone && <div className="form-input-error-message">{profileValidationErrors.phone}</div>}</div>
